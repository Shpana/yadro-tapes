...
public:
  ...
  auto AttachWorkload(std::weak_ptr<Workload> workload) -> void {
    workload_ = std::move(workload);
  }

  [[nodiscard]] auto size() const -> size_t override { return tape_size_; }
  [[nodiscard]] auto head_position() const -> size_t override { return head_position_; }

private:
  std::fstream tape_file_{};
  size_t tape_size_{};
  size_t head_position_{};
  std::weak_ptr<Workload> workload_{};
};
